...
export function AStar_typedArray(begin: point, end: point, isWall: ((point: point) => boolean)): point[] {
    ...
    interface myTypedArray extends Uint16Array {
    ...
    const open: myTypedArray = new Uint16Array(0xFFF);
    const closed: myTypedArray = new Uint16Array(0xFFFF);
    const closedBin = new Uint8Array(0xFFFFFF);
    const openRef = new Uint8Array(0xFFFFFF);
    open.emtpy = [];
    closed.emtpy = [];

    push(open, ...[begin.x, begin.y,/*h*/getDistance(begin, end),/*g*/0]);
    ...
    while (true) {
        ...
        const [x, y, g, h, fromX, fromY] = current;
        ...
        defaultNeighbours.forEach((nb) => {
            ...
            if (!offset) {
                ...
                push(
                    ...
                    nb.x,  // Our x
                    nb.y,  // y
                    g + getDistance({x, y}, nb), // g
                    getDistance(end, nb), // h
                    x, // our parent
                    y
                );
            } else if (open[offset + 2] >= g) {
                open[offset + 2] = g + getDistance({x, y}, nb); // Field g
                open[offset + 3] = getDistance(end, nb);          // field h
                open[offset + 4] = x;
                open[offset + 5] = y;
            }
        })
